...
export function CertificationManagement({ entries }: Props) {
  ...
  return <div className="space-y-4"><section className="app-card p-5"><form className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" onSubmit={submit}>
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Certificate no *" required value={form.certificateNo} onChange={(e) => setForm((f) => ({ ...f, certificateNo: e.target.value.toUpperCase() }))} />
    ...
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Product name" value={form.productName} onChange={(e) => setForm((f) => ({ ...f, productName: e.target.value }))} />
    ...
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Standard name" value={form.standardName} onChange={(e) => setForm((f) => ({ ...f, standardName: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Certification body" value={form.certBody} onChange={(e) => setForm((f) => ({ ...f, certBody: e.target.value }))} />
    ...
    <input type="date" className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.applicationDate ?? ""} onChange={(e) => setForm((f) => ({ ...f, applicationDate: e.target.value || null }))} />
    <input type="date" className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.issueDate ?? ""} onChange={(e) => setForm((f) => ({ ...f, issueDate: e.target.value || null }))} />
    <input type="date" className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.expiryDate ?? ""} onChange={(e) => setForm((f) => ({ ...f, expiryDate: e.target.value || null }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Report URL" value={form.reportUrl} onChange={(e) => setForm((f) => ({ ...f, reportUrl: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm sm:col-span-2 md:col-span-3 lg:col-span-4" placeholder="Notes" value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} />
