...
export function CertificationManagement({ entries }: Props) {
  ...
  </form>{message ? <p className="mt-2 text-sm text-red-600">{message}</p> : null}</section><section className="rounded-2xl border border-app-border/90 bg-app-surface p-5 shadow-sm overflow-x-auto"><table className="w-full min-w-[1100px] text-sm"><thead><tr className="border-b border-app-border/80 text-left text-app-muted"><th className="px-2 py-2">Cert no</th><th className="px-2 py-2">SKU</th><th className="px-2 py-2">Standard</th><th className="px-2 py-2">Status</th><th className="px-2 py-2">Expiry</th><th className="px-2 py-2">Owner</th><th className="px-2 py-2">Actions</th></tr></thead><tbody>{entries.map((e) => <tr key={e.id} className="border-b border-app-border/35"><td className="px-2 py-2">{e.certificateNo}</td><td className="px-2 py-2">{e.productSku}</td><td className="px-2 py-2">{e.standardName || "-"}</td><td className="px-2 py-2">{e.status}</td><td className="px-2 py-2">{e.expiryDate || "-"}</td><td className="px-2 py-2">{e.owner || "-"}</td><td className="px-2 py-2"><div className="flex gap-2"><button className="rounded border border-app-border px-2 py-1" onClick={() => edit(e)}>Edit</button><button className="rounded border border-red-200 px-2 py-1 text-red-700" onClick={() => remove(e.id)}>Delete</button></div></td></tr>)}</tbody></table></section></div>;
}
