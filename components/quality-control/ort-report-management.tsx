...
export function OrtReportManagement({ entries }: Props) {
  ...
  return <div className="space-y-4"><section className="app-card p-5"><form className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" onSubmit={submit}>
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="ORT no *" required value={form.ortNo} onChange={(e) => setForm((f) => ({ ...f, ortNo: e.target.value.toUpperCase() }))} />
    ...
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Batch no" value={form.batchNo} onChange={(e) => setForm((f) => ({ ...f, batchNo: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Factory" value={form.factory} onChange={(e) => setForm((f) => ({ ...f, factory: e.target.value }))} />
    <input type="number" min={0} className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Sample size" value={form.sampleSize} onChange={(e) => setForm((f) => ({ ...f, sampleSize: Number(e.target.value) || 0 }))} />
    <select className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.resultSummary} onChange={(e) => setForm((f) => ({ ...f, resultSummary: e.target.value as QcOrtResult }))}><option value="on_going">on_going</option><option value="pass">pass</option><option value="fail">fail</option></select>
    <input type="number" min={0} className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Fail count" value={form.failCount} onChange={(e) => setForm((f) => ({ ...f, failCount: Number(e.target.value) || 0 }))} />
    ...
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm sm:col-span-2 lg:col-span-2" placeholder="Test items" value={form.testItems} onChange={(e) => setForm((f) => ({ ...f, testItems: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm sm:col-span-2 lg:col-span-2" placeholder="Environment profile" value={form.environmentProfile} onChange={(e) => setForm((f) => ({ ...f, environmentProfile: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Duration" value={form.duration} onChange={(e) => setForm((f) => ({ ...f, duration: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" placeholder="Fail modes" value={form.failModes} onChange={(e) => setForm((f) => ({ ...f, failModes: e.target.value }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm sm:col-span-2 lg:col-span-2" placeholder="Action taken" value={form.actionTaken} onChange={(e) => setForm((f) => ({ ...f, actionTaken: e.target.value }))} />
    <input type="date" className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.startDate ?? ""} onChange={(e) => setForm((f) => ({ ...f, startDate: e.target.value || null }))} />
    <input type="date" className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm" value={form.endDate ?? ""} onChange={(e) => setForm((f) => ({ ...f, endDate: e.target.value || null }))} />
    <input className="min-w-0 rounded-lg border border-app-border px-3 py-2 text-sm sm:col-span-2 lg:col-span-2" placeholder="Report URL" value={form.reportUrl} onChange={(e) => setForm((f) => ({ ...f, reportUrl: e.target.value }))} />
